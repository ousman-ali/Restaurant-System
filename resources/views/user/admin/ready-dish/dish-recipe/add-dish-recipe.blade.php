...
@section('content')
    ...
    <ul class="nav nav-tabs">
        ...
        <li class="active">
            <a href="{{url('/ready-dish-recipe/'.$dish->id)}}" data-toggle="tab" aria-expanded="false">
                <span class="visible-xs"><i class="fa fa-photo"></i></span>
    ...
    <div class="tab-content">
        <div class="tab-pane active" id="home">
            <form class="form-horizontal" role="form" action="{{url('/save-ready-recipes/'.$dish->id)}}" id="updateDish"
                  method="post"
            ...
            <div class="table-responsive">
                <table class="table m-0">
                    ...
                    @foreach($dish->dishRecipes as $recipe)
                        <tr>
                            <td>{{ $count++ }}</td>
                            <td>{{ $recipe->product->product_name ?? 'N/A' }}</td>
                            <td>{{ $recipe->unit_needed }} - {{ $recipe->product->unit->unit ?? 'N/A' }}</td>
                            <td>{{ $recipe->child_unit_needed }} - {{ $recipe->product->unit->child_unit ?? 'N/A' }}</td>
